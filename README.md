...
在 LINE 輸入「去程」或「回程」，查詢今天、目前時間之後的最近 3 班台鐵。

列表只顯示最近三班、日期與現在時間。點 **1／2／3** 顯示車次、上車站與預估抵達時間，附「沒搭上」按鈕；按下後只顯示下一班出發與抵達時間。**已取消誤點追蹤、主動通知及上車／停止通知按鈕**。舊上車按鈕不再觸發，沒有背景輪詢或 Push 請求。沒有說明按鈕，但仍可手動輸入「說明」。

數字選擇以你最近收到的列表為準，按鈕則綁定原本的列表。列表有效 30 分鐘，換日或後端重啟後請重新查詢。**只接受完整指令**，不接受「我想搭回程」「回程選擇1」等句子，其他文字不回覆。不判斷是否已上車，也不會自動發送任何通知。
