...
test('完整 HTTP Webhook → TDX OAuth/車站/班表 → LINE reply，無外部網路', async t => {
  ...
  assert.equal(sent[0].messages[0].quickReply.items.length, 6);
  ...
  assert.match(messageText(sent[1].messages[0]), /【預計抵達大湖時間 17:48】/);
...
test('完整簽章 HTTP 選車即時估算保留，搭上功能已移除且停止可用', async t => {
  const delay = 6;
  const sent = [];
  const fakeFetch = async (input, init) => {
    const url = new URL(input);
  ...
  assert.deepEqual(selected.quickReply.items.map(x => x.action.label), ['知道', '沒搭上', '停止追蹤']);
  assert.match(messageText(selected), /預計抵達大湖時間 17:54/);
  const oldId = selected.quickReply.items.find(x => x.action.label === '停止追蹤').action.data.split(':')[2];
  ...
  await command('board-command', '搭上了'); await command('board-alias', '上車了'); await command('stop-text', '停止通知');
  assert.equal(sent.length, count);
  ...
  assert.match(messageText(sent.at(-1).data.messages[0]), /已停止/);
